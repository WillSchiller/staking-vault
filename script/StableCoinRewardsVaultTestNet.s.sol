...
contract Deploy is Script {
    ...
    function run() public {
        ...
        console2.log(
            "Deploying StableCoinRewardsVault with asset: %s, rewardToken: %s", address(asset), address(rewardToken)
        );

        vault = new StableCoinRewardsVault(
            asset, "NEXD Rewards Vault", "sNEXD", contractAdmin, epochManager, minAmount, maxAmount, maxPoolSize
        );
